    ...
    const handleDelete = (id) => {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('paymentmethod.delete', id));
            }
        });
    }
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div className="border-b border-slate-100">
                            <div className="p-6 text-gray-900 flex items-center justify-between">
                                <h4 className="text-lg font-semibold">Data Payment Method</h4>
                                <LinkPrimary href={route('paymentmethod.add')}>Add New</LinkPrimary>
                            </div>
                        </div>
                                            ...
                                            props.data.map((item, index) => (
                                                <tr key={index} className="bg-white border-b border-gray-200">
                                                    ...
                                                    <th
                                                        className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                                        {
                                                            item.is_default && <div className="flex items-center gap-1"><RiCheckboxBlankCircleFill className="text-green-700"/> <p className="text-green-800">Default</p></div>
                                                        }
                                                    </th>
                                                    <td className="px-6 py-4 text-right">
                                                        <div className="flex items-center gap-1 justify-end">
                                                            <LinkSecondary href={route('paymentmethod.edit', item.id)} data-tooltip-id="my-tooltip" data-tooltip-content="Edit Data"><RiEdit2Line/></LinkSecondary>
